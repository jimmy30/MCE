...
require_once($_SERVER['DOCUMENT_ROOT']."/GUIService/admin/Reports/PlaceCastDownloadReportService.php");
require_once($_SERVER['DOCUMENT_ROOT']."/IncludeFiles/PHP/Najax/najax.php");

try
{
$objReg = new PlaceCastDownloadReportService();
...
<script type="text/javascript" src="/IncludeFiles/Javascript/Admin/Reports/PlaceCastDownloadReport.js"></script>


<script language="javascript">
var obj = <?= NAJAX_Client::register(new PlaceCastDownloadReportService()) ?>;
...
var ToDate = new Date();
	if(ValidateForm())
	{
		...
		obj.GetPlaceCastDownloadReport(FromDate,ToDate,null,function(result) { parseXMLResponse(result);});
	}
}
...
function getReportByCountry()
{
	...
	obj.GetPlaceCastDownloadReport(null,null,document.getElementById("cmbCountryRegion").value,function(result) { parseXMLResponse(result);});
...
function getReportAll()
{
	...
	document.getElementById("hiddenToDate").value='';
	document.getElementById("hiddenCountryId").value='';	
	obj.GetPlaceCastDownloadReport(null,null,null,function(result) { parseXMLResponse(result);});
}
...
function parseXMLResponse(pResponse)
{
		...
		strHtml+='<table width="90%" border="0" align="center" cellpadding="4" cellspacing="1" class="RegistrationTabBorder">';
          strHtml+='<tr class="RegistrationCellBg">';
            strHtml+='<td colspan="3" class="RegistrationTitleTextSmall">PlaceCast Downloads Report</td>';
          strHtml+='</tr>';
          strHtml+='<tr class="RegistrationBodyText">';
            strHtml+='<td bgcolor="#eeeeee"><strong>PlaceCast ID</strong></td>';
            strHtml+='<td bgcolor="#eeeeee"><strong>Name</strong></td>';
            strHtml+='<td bgcolor="#eeeeee"><strong>No. of Downloads</strong></td>';
            strHtml+='</tr>';
		...
		if (window.ActiveXObject)
		{
			if(XMLDoc.loadXML(strResonse)==true)
			{
				...
				for(var i=0;i<intCount;i++)
				{
					...
					var PlaceCastCount=rootNode.selectSingleNode("PlaceCastList").childNodes.item(i).childNodes.item(0).text;
					var PlaceCastId=rootNode.selectSingleNode("PlaceCastList").childNodes.item(i).childNodes.item(1).text;

					var PlaceCastName=rootNode.selectSingleNode("PlaceCastList").childNodes.item(i).childNodes.item(2).text;



				  strHtml+='<tr class="RegistrationBodyText">';
					strHtml+='<td width="30%" bgcolor="#eeeeee">'+PlaceCastId+'</td>';
					strHtml+='<td width="30%" bgcolor="#eeeeee">'+PlaceCastName+'</td>';
					strHtml+='<td width="30%" bgcolor="#eeeeee">'+PlaceCastCount+'</td>';
					strHtml+='</tr>';
					
				TotalPlaceCastCount+=parseInt(PlaceCastCount);
		...
		else
		{
			...
			if(strResonse!="")
			{
				...
				var strPlaceCastDownload = xmlDoc.getElementsByTagName('PlaceCastDownload');
				var intCount=strPlaceCastDownload[0].getElementsByTagName("NoOfRecord")[0].textContent;
				...
				for(var i=0;i<intCount;i++)
				{
					...
					var PlaceCastCount=strPlaceCastList[i].getElementsByTagName("PlaceCastCount")[0].textContent;
					var PlaceCastId=strPlaceCastList[i].getElementsByTagName("PlaceCastId")[0].textContent;
					var PlaceCastName=strPlaceCastList[i].getElementsByTagName("PlaceCastName")[0].textContent;
          ...
          strHtml+='<tr class="RegistrationBodyText">';
            ...
            strHtml+='<td bgcolor="#eeeeee"><strong>&nbsp;</strong></td>';
            strHtml+='<td bgcolor="#eeeeee"><strong>'+TotalPlaceCastCount+'</strong></td>';
            strHtml+='</tr>';
          strHtml+='<tr class="RegistrationBodyText">';
            strHtml+='<td colspan="3" bgcolor="#eeeeee"><div align="center"><strong>Grand Total = '+TotalPlaceCastCount+'</strong></div></td>';
            strHtml+='</tr>';
            strHtml+='</table>';
...
<table width="845" border="0" align="center" cellpadding="0" cellspacing="0" class="RegistrationTabBorder" height="100%">
  ...
  <form action="PlaceCastDownloadReportPrint.php" method="post" name="frmUserReportPrint" target="_blank">
