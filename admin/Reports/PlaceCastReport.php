...
require_once($_SERVER['DOCUMENT_ROOT']."/GUIService/admin/Reports/PlaceCastReportService.php");
require_once($_SERVER['DOCUMENT_ROOT']."/IncludeFiles/PHP/Najax/najax.php");

try
{
$objReg = new PlaceCastReportService();
...
<script type="text/javascript" src="/IncludeFiles/Javascript/Admin/Reports/PlaceCastReport.js"></script>


<script language="javascript">
var obj = <?= NAJAX_Client::register(new PlaceCastReportService()) ?>;
...
var ToDate = new Date();
	if(ValidateForm())
	{
		...
		obj.GetPlaceCastReport(FromDate,ToDate,null,function(result) { parseXMLResponse(result);});
	}
}
...
{
	document.getElementById("loading").style.display='inline';
	document.getElementById("hiddenCountryId").value=document.getElementById("cmbCountryRegion").value;
	document.getElementById("hiddenFromDate").value='';
	document.getElementById("hiddenToDate").value='';
		
	obj.GetPlaceCastReport(null,null,document.getElementById("cmbCountryRegion").value,function(result) { parseXMLResponse(result);});
...
function getReportAll()
{
	...
	document.getElementById("hiddenToDate").value='';
	document.getElementById("hiddenCountryId").value='';	
	obj.GetPlaceCastReport(null,null,null,function(result) { parseXMLResponse(result);});
}
...
function parseXMLResponse(pResponse)
{
		...
		strHtml+='<table width="90%" border="0" align="center" cellpadding="4" cellspacing="1" class="RegistrationTabBorder">';
          strHtml+='<tr class="RegistrationCellBg">';
            strHtml+='<td colspan="3" class="RegistrationTitleTextSmall">PlaceCast Report</td>';
          strHtml+='</tr>';
          strHtml+='<tr class="RegistrationBodyText">';
            strHtml+='<td bgcolor="#eeeeee"><strong>Country</strong></td>';
            strHtml+='<td bgcolor="#eeeeee"><strong>Active</strong></td>';
            strHtml+='<td bgcolor="#eeeeee"><strong>Inactive</strong></td>';
            strHtml+='</tr>';
		...
		if (window.ActiveXObject)
		{
			if(XMLDoc.loadXML(strResonse)==true)
			{
				...
				for(var i=0;i<intCount;i++)
				{
					...
					var ActiveCount=rootNode.selectSingleNode("CountryList").childNodes.item(i).childNodes.item(0).childNodes.item(0).text;
					var ActiveCountryName=rootNode.selectSingleNode("CountryList").childNodes.item(i).childNodes.item(0).childNodes.item(1).text;
					var InActiveCount=rootNode.selectSingleNode("CountryList").childNodes.item(i).childNodes.item(1).childNodes.item(0).text;
					var InActiveCountryName=rootNode.selectSingleNode("CountryList").childNodes.item(i).childNodes.item(1).childNodes.item(1).text;


				  strHtml+='<tr class="RegistrationBodyText">';
					strHtml+='<td width="30%" bgcolor="#eeeeee">'+ActiveCountryName+'</td>';
					strHtml+='<td width="30%" bgcolor="#eeeeee">'+ActiveCount+'</td>';
					strHtml+='<td width="30%" bgcolor="#eeeeee">'+InActiveCount+'</td>';
					strHtml+='</tr>';
					
				activeSum+=parseInt(ActiveCount);
				inActiveSum+=parseInt(InActiveCount);
		...
		else
		{
			...
			if(strResonse!="")
			{
				...
				var intCount=strPlaceCast[0].getElementsByTagName("NoOfRecord")[0].textContent;
	
				var strCountryList = xmlDoc.getElementsByTagName('Country');
	
				for(var i=0;i<intCount;i++)
				{
	
					var ActiveCount=strCountryList[i].getElementsByTagName("RecordCountActive")[0].textContent;
					var ActiveCountryName=strCountryList[i].getElementsByTagName("CountryNameActive")[0].textContent;
					var InActiveCount=strCountryList[i].getElementsByTagName("RecordCountInActive")[0].textContent;
					var InActiveCountryName=strCountryList[i].getElementsByTagName("CountryNameInActive")[0].textContent;
			...
			total=activeSum+inActiveSum;
			
          strHtml+='<tr class="RegistrationBodyText">';
            strHtml+='<td bgcolor="#eeeeee"><strong>Total</strong></td>';
            strHtml+='<td bgcolor="#eeeeee"><strong>'+activeSum+'</strong></td>';
            strHtml+='<td bgcolor="#eeeeee"><strong>'+inActiveSum+'</strong></td>';
            strHtml+='</tr>';
          strHtml+='<tr class="RegistrationBodyText">';
            strHtml+='<td colspan="3" bgcolor="#eeeeee"><div align="center"><strong>Grand Total = '+total+'</strong></div></td>';
            strHtml+='</tr>';
            strHtml+='</table>';
...
function setFields(intAction)
{
	...
	document.getElementById("cmbCountryRegion").disabled = true;
	...
	{
		document.getElementById("cmbCountryRegion").disabled = false;	
	}
	else if(intAction==3)
	{
	}
...
<table width="845" border="0" align="center" cellpadding="0" cellspacing="0" class="RegistrationTabBorder" height="100%">
  ...
  <input name="txtToYear" type="text" id="txtToYear" size="4" maxlength="6" value="[Year]" onFocus="if(document.frmReport.txtToYear.value=='[Year]') document.frmReport.txtToYear.value=''" onBlur="if(document.frmReport.txtToYear.value=='') document.frmReport.txtToYear.value='[Year]'">
        ...
        <tr>
          <td class="RegistrationBodyText">
            <div align="left">
              <input name="rdoSearchType" type="radio" value="2" onClick="setFields(2)">
              Serch By Country: </div></td>
          <td class="RegistrationBodyText">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <select name="cmbCountryRegion" id="cmbCountryRegion" style="width:220px">
  ...
  <form action="PlaceCastReportPrint.php" method="post" name="frmUserReportPrint" target="_blank">
