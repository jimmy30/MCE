...
require_once($_SERVER['DOCUMENT_ROOT']."/GUIService/admin/Reports/WaypointDownloadReportService.php");
require_once($_SERVER['DOCUMENT_ROOT']."/IncludeFiles/PHP/Najax/najax.php");

try
{
$objReg = new WaypointDownloadReportService();
...
<script type="text/javascript" src="/IncludeFiles/Javascript/Admin/Reports/WaypointDownloadReport.js"></script>


<script language="javascript">
var obj = <?= NAJAX_Client::register(new WaypointDownloadReportService()) ?>;
...
var ToDate = new Date();
	if(ValidateForm())
	{
		...
		obj.GetWaypointDownloadReport(FromDate,ToDate,null,function(result) { parseXMLResponse(result);});
	}
}
...
function getReportByCountry()
{
	...
	obj.GetWaypointDownloadReport(null,null,document.getElementById("cmbCountryRegion").value,function(result) { parseXMLResponse(result);});
...
function getReportAll()
{
	...
	document.getElementById("hiddenToDate").value='';
	document.getElementById("hiddenCountryId").value='';	
	obj.GetWaypointDownloadReport(null,null,null,function(result) { parseXMLResponse(result);});
}
...
function parseXMLResponse(pResponse)
{
		...
		strHtml+='<table width="90%" border="0" align="center" cellpadding="4" cellspacing="1" class="RegistrationTabBorder">';
          strHtml+='<tr class="RegistrationCellBg">';
            strHtml+='<td colspan="3" class="RegistrationTitleTextSmall">Waypoint Downloads Report</td>';
          strHtml+='</tr>';
          strHtml+='<tr class="RegistrationBodyText">';
            strHtml+='<td bgcolor="#eeeeee"><strong>Waypoint ID</strong></td>';
		...
		if (window.ActiveXObject)
		{
			if(XMLDoc.loadXML(strResonse)==true)
			{
				...
				for(var i=0;i<intCount;i++)
				{
					...
					var WaypointCount=rootNode.selectSingleNode("WaypointList").childNodes.item(i).childNodes.item(0).text;
					var WaypointId=rootNode.selectSingleNode("WaypointList").childNodes.item(i).childNodes.item(1).text;

					var WaypointName=rootNode.selectSingleNode("WaypointList").childNodes.item(i).childNodes.item(2).text;



				  strHtml+='<tr class="RegistrationBodyText">';
					strHtml+='<td width="30%" bgcolor="#eeeeee">'+WaypointId+'</td>';
					strHtml+='<td width="30%" bgcolor="#eeeeee">'+WaypointName+'</td>';
					strHtml+='<td width="30%" bgcolor="#eeeeee">'+WaypointCount+'</td>';
					strHtml+='</tr>';
					
				TotalWaypointCount+=parseInt(WaypointCount);
		...
		else
		{
			...
			if(strResonse!="")
			{
				...
				var strWaypointDownload = xmlDoc.getElementsByTagName('WaypointDownload');
				var intCount=strWaypointDownload[0].getElementsByTagName("NoOfRecord")[0].textContent;
				...
				for(var i=0;i<intCount;i++)
				{
					...
					var WaypointCount=strWaypointList[i].getElementsByTagName("WaypointCount")[0].textContent;
					var WaypointId=strWaypointList[i].getElementsByTagName("WaypointId")[0].textContent;
					var WaypointName=strWaypointList[i].getElementsByTagName("WaypointName")[0].textContent;
					  strHtml+='<tr class="RegistrationBodyText">';
						strHtml+='<td width="30%" bgcolor="#eeeeee">'+WaypointId+'</td>';
						strHtml+='<td width="30%" bgcolor="#eeeeee">'+WaypointName+'</td>';
						strHtml+='<td width="30%" bgcolor="#eeeeee">'+WaypointCount+'</td>';
          ...
          strHtml+='<tr class="RegistrationBodyText">';
            ...
            strHtml+='<td bgcolor="#eeeeee"><strong>'+TotalWaypointCount+'</strong></td>';
            strHtml+='</tr>';
          strHtml+='<tr class="RegistrationBodyText">';
            strHtml+='<td colspan="3" bgcolor="#eeeeee"><div align="center"><strong>Grand Total = '+TotalWaypointCount+'</strong></div></td>';
            strHtml+='</tr>';
            strHtml+='</table>';
...
<table width="845" border="0" align="center" cellpadding="0" cellspacing="0" class="RegistrationTabBorder" height="100%">
  ...
  <form action="WaypointDownloadReportPrint.php" method="post" name="frmUserReportPrint" target="_blank">
