...
require_once($_SERVER['DOCUMENT_ROOT']."/GUIService/admin/Reports/WaypointReportService.php");
require_once($_SERVER['DOCUMENT_ROOT']."/IncludeFiles/PHP/Najax/najax.php");

try
{
$objReg = new WaypointReportService();
...
<script type="text/javascript" src="/IncludeFiles/Javascript/Admin/Reports/WaypointReport.js"></script>


<script language="javascript">
var obj = <?= NAJAX_Client::register(new WaypointReportService()) ?>;
...
var ToDate = new Date();
	if(ValidateForm())
	{
		...
		obj.GetWaypointReport(FromDate,ToDate,null,function(result) { parseXMLResponse(result);});
	}
}
...
function getReportByCountry()
{
	...
	obj.GetWaypointReport(null,null,document.getElementById("cmbCountryRegion").value,function(result) { parseXMLResponse(result);});
...
function getReportAll()
{
	...
	obj.GetWaypointReport(null,null,null,function(result) { parseXMLResponse(result);});
}
...
function parseXMLResponse(pResponse)
{
		...
		strHtml+='<table width="90%" border="0" align="center" cellpadding="4" cellspacing="1" class="RegistrationTabBorder">';
          strHtml+='<tr class="RegistrationCellBg">';
            strHtml+='<td colspan="3" class="RegistrationTitleTextSmall">Waypoint Report</td>';
          strHtml+='</tr>';
		...
		else
		{
			...
			if(strResonse!="")
			{
				...
				var intCount=strWaypoint[0].getElementsByTagName("NoOfRecord")[0].textContent;
...
<table width="845" border="0" align="center" cellpadding="0" cellspacing="0" class="RegistrationTabBorder" height="100%">
  ...
  <form action="WaypointReportPrint.php" method="post" name="frmUserReportPrint" target="_blank">
