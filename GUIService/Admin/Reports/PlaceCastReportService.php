...
require_once($_SERVER['DOCUMENT_ROOT']."/Database/DAO/Admin/Reports/PlaceCastDAO.php");
...
class PlaceCastReportService
{
	/// Declaring Response variable
	var $strResponse;
	
	/// RECORD COUNT BY ACCOUNT TYPE AND STATUS
	function GetPlaceCastReport($pFromDate, $pToDate, $pCountryId)
	{
		$objPlaceCastDAO = new PlaceCastDAO();
		$arrayResult = $objPlaceCastDAO->GetPlaceCastReport($pFromDate, $pToDate, $pCountryId);
		$xmlResponse="";
...
//	fclose($handle);
	...
	{
		NAJAX_Client::mapMethods($this, array('GetPlaceCastReport'));

		NAJAX_Client::publicMethods($this, array('GetPlaceCastReport'));
	}
