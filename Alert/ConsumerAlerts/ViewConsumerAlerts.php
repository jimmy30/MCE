...
require_once($_SERVER['DOCUMENT_ROOT']."/GUIService/Alert/ConsumerAlerts/ViewConsumerAlertService.php");
require_once($_SERVER['DOCUMENT_ROOT']."/IncludeFiles/PHP/Najax/najax.php");

try
{
	$objReg = new ViewConsumerAlertService();
	NAJAX_Server::allowClasses("ViewConsumerAlertService");
...
<title>MCE-Add Consumer Alert</title>
<!-- InstanceEndEditable -->
...
var obj = <?= NAJAX_Client::register(new ViewConsumerAlertService()) ?>;
var intPageNo=1;
...
var i;
	...
	function getListConsumerAlerts()
	{
		...
		obj.GetListByConsumerId(<?php echo $_SESSION[sessionKeys::USER_ID]?>,arrayPagingLimit[0],arrayPagingLimit[1],1,intSortListBy,function(result){parseXMLResponse(result);});
...
//		obj.AlertRecordCount(1,function(result){getPaging(result);});
	...
	function parseXMLResponse(pResponse)
	{
			...
			if(strResonse!=null)
			{
				...
				if(XMLDoc.loadXML(strResonse)==true)
				{
					...
					for(i=0;i< intNoOfRecords;i++)	
					{
						...
						intConsumerAlertId=rootNode.selectSingleNode("ConsumerAlertList").childNodes.item(i).childNodes.item(0).text;
						strCountryName=rootNode.selectSingleNode("ConsumerAlertList").childNodes.item(i).childNodes.item(2).text;				
						intAdd=rootNode.selectSingleNode("ConsumerAlertList").childNodes.item(i).childNodes.item(3).text;				
						intModify=rootNode.selectSingleNode("ConsumerAlertList").childNodes.item(i).childNodes.item(4).text;				
						intIsActive=rootNode.selectSingleNode("ConsumerAlertList").childNodes.item(i).childNodes.item(5).text;				
		...
		else
		{
			if(strResonse!=null)
			{
				...
				var strConsumerAlerts = xmlDoc.getElementsByTagName('ConsumerAlerts');
				var intNoOfRecord = strConsumerAlerts[0].getElementsByTagName('NoOfRecords')[0].textContent;
				var strConsumerAlertList = xmlDoc.getElementsByTagName('ConsumerAlert');
				...
				for (var i = 0; i < intNoOfRecord; i++) 
				{
					...
					if((Math.round(i - (Math.floor(i/2)*2)))==0)
				 	...
				 	intConsumerAlertId= strConsumerAlertList[i].getElementsByTagName('ConsumerAlertId')[0].textContent;
	...
	function DelteConsumerAlert(ConsumerAlertId)
	{
		<?php 
		if(isset($_SESSION[sessionKeys::USER_ID]) && $_SESSION[sessionKeys::USER_ID]!="") 
		{
		?>
			...
			{
				obj.ConsumerAlertDeleteById(ConsumerAlertId,function(result)
				{
		...
		if (window.ActiveXObject)
		{
			if(XMLDoc.loadXML(strResonse)==true)
			{
				...
				if(strStatus=="ok")
				{
					...
					document.getElementById("divError").innerHTML=CONSUMER_ALERT_SUCCESSFULLY_DELETED;
...
<table width="670" height="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="RegistrationTabBorder">
  <form name="frmConsumerAlert" action="" method="post" onSubmit="">
    <tr  class="RegistrationCellBg">
      <td height="27" colspan="2"><p class="RegistrationTitleText">&nbsp;&nbsp;&nbsp;View Consumer Alerts</p></td>
      </tr>
    <tr>
...
<div align="center"><input type="button" class="RegistrationButton" onClick="location.href='AddConsumerAlerts.php'" value="Add New">&nbsp;&nbsp;</div>	  </td>
    </tr>
    <tr align="left">
      <td colspan="2">&nbsp;</td>
    </tr>
