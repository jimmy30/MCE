...
require_once($_SERVER['DOCUMENT_ROOT']."/GUIService/Alert/ConsumerAlerts/EditConsumerAlertsService.php");
...
try
{
	$objReg = new EditConsumerAlertsService();
	NAJAX_Server::allowClasses("EditConsumerAlertsService");
...
<script type="text/javascript" src="/IncludeFiles/Javascript/Alert/ConsumerAlerts/EditConsumerAlerts.js"></script>
<script type="text/javascript" src="/IncludeFiles/Javascript/Messages.js"></script>
<script language="javascript">

var obj = <?= NAJAX_Client::register(new EditConsumerAlertsService()) ?>;
...
function createXml()
{
	if(ValidateForm())
	{
		try
		{
			...
			if(document.frmEditConsumerAlert.rdoCountry[0].checked)
				country = document.frmEditConsumerAlert.rdoCountry[0].value;
			else if(document.frmEditConsumerAlert.rdoCountry[1].checked)
				country = document.frmEditConsumerAlert.cmbCountryRegion.value;
			
			if(document.frmEditConsumerAlert.chkAdd.checked)
				add = document.frmEditConsumerAlert.chkAdd.value;
				
			if(document.frmEditConsumerAlert.chkModify.checked)
				modify = document.frmEditConsumerAlert.chkModify.value;
			
			if(document.frmEditConsumerAlert.rdoStatus[0].checked)
				status = document.frmEditConsumerAlert.rdoStatus[0].value;
			else if(document.frmEditConsumerAlert.rdoStatus[1].checked)
				status = document.frmEditConsumerAlert.rdoStatus[1].value;			
...
function getConsumerAlert()
{
	...
	var strResonse=obj.GetConsumerAlertById('<?php echo $_REQUEST["id"]?>',1,function(result){parseXMLConsumerAlert(result);});
...
function parseXMLConsumerAlert(pResponse)
{
		...
		if (window.ActiveXObject)
		{
			if(XMLDoc.loadXML(strResonse)==true)
			{
				...
				if(rootNode.selectSingleNode("CountryId").text==-1)
				{
					...
					document.frmEditConsumerAlert.rdoCountry[0].checked = true;
				}
				else
				{
					document.frmEditConsumerAlert.rdoCountry[1].checked = true;
					document.frmEditConsumerAlert.cmbCountryRegion.value = rootNode.selectSingleNode("CountryId").text
				}
				
				if(rootNode.selectSingleNode("Add").text==1)
					document.frmEditConsumerAlert.chkAdd.checked = true;

				if(rootNode.selectSingleNode("Modify").text==1)
					document.frmEditConsumerAlert.chkModify.checked = true;

				if(rootNode.selectSingleNode("IsActive").text==1)
					document.frmEditConsumerAlert.rdoStatus[0].checked = true;
				else if(rootNode.selectSingleNode("IsActive").text==0)
					document.frmEditConsumerAlert.rdoStatus[1].checked = true;
		...
		{
			var xmlDoc = XMLDoc.parseFromString(strResonse, "application/xml");
			var strConsumerAlert = xmlDoc.getElementsByTagName('ConsumerAlert');
				...
				{
					document.frmEditConsumerAlert.rdoCountry[1].checked = true;
					document.frmEditConsumerAlert.cmbCountryRegion.value = intCountryId;
				}
...
<table width="670" border="0" align="center" cellpadding="0" cellspacing="0" class="RegistrationTabBorder">
  <form name="frmEditConsumerAlert" action="" method="post" onSubmit="">
    <tr  class="RegistrationCellBg">
      <td align="left" colspan="2" valign="middle"><p class="RegistrationTitleText">&nbsp;&nbsp;&nbsp;Edit Alert  </p></td>
    </tr>
    <tr>
    ...
    <tr align="left">
      <td colspan="2"> <table width="660" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            ...
            <td width="154" align="left" valign="top"><input name="Submit" type="Button" class="RegistrationButton" id="Submit" value="Update Alert" onClick="createXml()"></td>
