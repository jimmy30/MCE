...
require_once($_SERVER['DOCUMENT_ROOT']."/GUIService/Alert/SmsAlerts/AddSmsAlertsService.php");
...
try
{
	$objReg = new AddSmsAlertsService();
	NAJAX_Server::allowClasses("AddSmsAlertsService");
...
<script type="text/javascript" src="/IncludeFiles/Javascript/Alert/SmsAlerts/AddSmsAlerts.js"></script>
<script type="text/javascript" src="/IncludeFiles/Javascript/Messages.js"></script>
<script language="javascript">

var obj = <?= NAJAX_Client::register(new AddSmsAlertsService()) ?>;
...
function createXml()
{
	if(ValidateForm())
	{
		try
		{
			...
			if(document.frmAddSmsAlert.rdoCountry[0].checked)
				country = document.frmAddSmsAlert.rdoCountry[0].value;
			else if(document.frmAddSmsAlert.rdoCountry[1].checked)
				country = document.frmAddSmsAlert.cmbCountryRegion.value;
			
			if(document.frmAddSmsAlert.chkAdd.checked)
				add = document.frmAddSmsAlert.chkAdd.value;
				
			if(document.frmAddSmsAlert.chkModify.checked)
				modify = document.frmAddSmsAlert.chkModify.value;
			
			if(document.frmAddSmsAlert.rdoStatus[0].checked)
				status = document.frmAddSmsAlert.rdoStatus[0].value;
			else if(document.frmAddSmsAlert.rdoStatus[1].checked)
				status = document.frmAddSmsAlert.rdoStatus[1].value;			
		...
		if (window.ActiveXObject)
		{
			if(XMLDoc.loadXML(strResonse)==true)
			{
				...
				if(strStatus=="ok")
				{
					...
					document.getElementById("divError").innerHTML=SMS_ALERT_SUCCUSSFULLY_ADDED;
				}
				else if(strStatus==-1)
				{
					document.getElementById("strip_image").src="/ImageFiles/common/error.gif";
					document.getElementById("divError").innerHTML=SMS_ALL_COUNRY_ERROR;

				}
				else if(strStatus==0)
				{
					document.getElementById("strip_image").src="/ImageFiles/common/error.gif";
					document.getElementById("divError").innerHTML=SMS_SAME_COUNRY_ERROR;
				}
...
<table width="670" border="0" align="center" cellpadding="0" cellspacing="0" class="RegistrationTabBorder">
  <form name="frmAddSmsAlert" action="" method="post" onSubmit="">
    <tr  class="RegistrationCellBg">
      <td align="left" colspan="2" valign="middle"><p class="RegistrationTitleText">&nbsp;&nbsp;&nbsp;Add Sms Alert  </p></td>
    </tr>
    <tr>
    ...
    <tr align="left">
      <td colspan="2"> <table width="660" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            ...
            <td width="154" align="left" valign="top"><input name="Submit" type="Button" class="RegistrationButton" id="Submit" value="Add Sms Alert" onClick="createXml()"></td>
