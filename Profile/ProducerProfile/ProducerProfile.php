<?php 
require_once($_SERVER['DOCUMENT_ROOT']."/GUIService/Profile/ProducerProfile/ProducerProfileService.php");
...
try
{
	$objReg = new ProducerProfileService();
	NAJAX_Server::allowClasses("ProducerProfileService");
...
<script type="text/javascript" src="/IncludeFiles/Javascript/Profile/ProducerProfile/ProducerProfile.js"></script>
...
var obj = <?= NAJAX_Client::register(new ProducerProfileService()) ?>;
		...
		if (window.ActiveXObject)
		{
			if(XMLDoc.loadXML(strResonse)==true)
			{
				...
				if(strStatus=="ok")
				{
					...
					document.getElementById("divError").innerHTML=PRODUCER_PROFILE_SUCCUSSFULLY_UPDATED;
				}
...
{
	var strResonse=obj.GetProducerListByEmail('<?php echo $_SESSION[sessionKeys::USER_EMAIL]?>',function(result){parseXMLProducerList(result);});
}

function parseXMLProducerList(pResponse)
{
		var XMLDoc =GetXmlHttpObject();
		var strResonse=pResponse;
		XMLDoc.async = "false";
		if (window.ActiveXObject)
		{
			if(XMLDoc.loadXML(strResonse)==true)
			{
				...
				rootNode=XMLDoc.documentElement;
				document.getElementById("txtFirstName").value =rootNode.selectSingleNode("ProducerFristName").text;
				document.getElementById("txtLastName").value = rootNode.selectSingleNode("ProducerLastName").text;
				document.getElementById("txtAddress1").value = rootNode.selectSingleNode("ProducerAddress").text;			
				document.getElementById("txtCity").value = rootNode.selectSingleNode("ProducerCity").text;						
				document.getElementById("cmbCountryRegion").value = rootNode.selectSingleNode("ProducerCountryId").text;
				getListStates();			
				document.getElementById("cmbState").value = rootNode.selectSingleNode("ProducerStateId").text;
				document.getElementById("txtZipCode").value = rootNode.selectSingleNode("ProducerZipCode").text;									
	
				var arrTelephone = new Array();
				arrTelephone = rootNode.selectSingleNode("ProducerTelephone1").text.split("-");
				...
				arrDOB = rootNode.selectSingleNode("ProducerDateOfBirth").text.split("-");
				...
				document.getElementById("cmbSecretQuestion").value = rootNode.selectSingleNode("ProducerSecretQuestion").text;									
				document.getElementById("txtAnswer").value = rootNode.selectSingleNode("ProducerAnswer").text;									
				document.getElementById("cmbAccountType").value = rootNode.selectSingleNode("ProducerAccountType").text;									
		...
		{
			var xmlDoc = XMLDoc.parseFromString(strResonse, "application/xml");
			var ConsumerList = xmlDoc.getElementsByTagName('ProducerList');
			document.getElementById("txtFirstName").value =ConsumerList[0].getElementsByTagName("ProducerFristName")[0].textContent;
			document.getElementById("txtLastName").value = ConsumerList[0].getElementsByTagName("ProducerLastName")[0].textContent;
			document.getElementById("txtAddress1").value = ConsumerList[0].getElementsByTagName("ProducerAddress")[0].textContent;
			document.getElementById("txtCity").value = ConsumerList[0].getElementsByTagName("ProducerCity")[0].textContent;
			document.getElementById("cmbCountryRegion").value = ConsumerList[0].getElementsByTagName("ProducerCountryId")[0].textContent;
			getListStates();
			document.getElementById("cmbState").value =ConsumerList[0].getElementsByTagName("ProducerStateId")[0].textContent;
			document.getElementById("txtZipCode").value = ConsumerList[0].getElementsByTagName("ProducerZipCode")[0].textContent;
			var arrTelephone = new Array();
			arrTelephone = ConsumerList[0].getElementsByTagName("ProducerTelephone1")[0].textContent.split("-");
			document.getElementById("txtTelephone1").value = arrTelephone[0];												
			document.getElementById("txtExtension").value = arrTelephone[1];
			var arrDOB = new Array();
			arrDOB = ConsumerList[0].getElementsByTagName("ProducerDateOfBirth")[0].textContent.split("-");
			document.getElementById("cmbDay").value = arrDOB[2];
			document.getElementById("cmbMonth").value = arrDOB[1];			
			document.getElementById("cmbYear").value = arrDOB[0];						
			document.getElementById("cmbSecretQuestion").value = ConsumerList[0].getElementsByTagName("ProducerSecretQuestion")[0].textContent;									
			document.getElementById("txtAnswer").value = ConsumerList[0].getElementsByTagName("ProducerAnswer")[0].textContent;									
			document.getElementById("cmbAccountType").value =ConsumerList[0].getElementsByTagName("ProducerAccountType")[0].textContent;						
		}	
}
      ...
      <td colspan="2"><table width="638" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td align="left" valign="middle">
              ...
              <a href="#" class="LinkSmall" onClick="MM_openBrWindow('/Registration/ProducerRegistration/AccountTypeInformation.php','AccountType','status=yes,scrollbars=yes,resizable=yes,width=800,height=580')">Learn More </a> </td>
          </tr>
          <tr>
            <td align="left" valign="middle">&nbsp;</td>
          </tr>
