...
/*=================================================================================
		Function Name 	: GetWebsiteHitsReport
...
=================================================================================*/
		function GetWebsiteHitsReport($pFromDate, $pToDate, $pCountryId)
		{
			$strQuery = "CALL WEBSITE_HITS_RECORD_COUNT_REPORT(?,?)";
			$objStatement=$this->objDB->executePrepare($strQuery);
			$objStatement->bind_param('ss',$pFromDate, $pToDate);
			$objStatement->execute();

 			$objStatement->bind_result($numRows, $ClientIP);
			$i=0;

			while($objStatement->fetch())
			{
				$arrayResult[$i][0]=$numRows;
				$arrayResult[$i][1]=$ClientIP;
				$i++;
			}
