...
=================================================================================*/
		...
		{
			$strQuery = "CALL WAYPOINT_DOWNLOAD_RECORD_COUNT_REPORT(?,?)";
			$objStatement=$this->objDB->executePrepare($strQuery);
			$objStatement->bind_param('ss',$pFromDate, $pToDate);
			$objStatement->execute();

 			$objStatement->bind_result($numRows, $WaypointId, $WaypointName);
			$i=0;

			while($objStatement->fetch())
			{
				$arrayResult[$i][0]=$numRows;
				$arrayResult[$i][1]=$WaypointId;
				$arrayResult[$i][2]=$WaypointName;
				$i++;
			}
